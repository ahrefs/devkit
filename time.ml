...
let fast_to_string =
  ...
  let template = "2018-__-__T__:__:__" in
  ...
  fun ~gmt f ->
    ...
    replace_year_2018 s (t.tm_year + 1900);
...
(** YYYYMMDDhhmmss *)
let format_date8hms =
  let template = "2018__________" in
  fun t ->
    let open Unix in
    let s = Bytes.of_string template in
    replace_year_2018 s (t.tm_year + 1900);
