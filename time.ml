...
let fast_to_string =
  ...
  let template = "2021-__-__T__:__:__" in
...
(** YYYYMMDDhhmmss *)
let format_date8hms =
  let template = "2021__________" in
  fun t ->
    let open Unix in
    let s = Bytes.of_string template in
    replace_year_2021 s t;
...
(** YYYY-MM-DD hh:mm:ss *)
let format_basic =
  let template = "2021-__-__ __:__:__" in
  fun t ->
    let open Unix in
    let s = Bytes.of_string template in
    replace_year_2021 s t;
