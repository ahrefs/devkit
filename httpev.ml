...
type client = {
  ...
  server : server;
  logev : Logev.t; (** Wide event for the request *)
}
...
let setup_server_fd fd config answer =
  ...
  Tcp.handle config.events fd begin fun (fd,sockaddr) ->
    ...
    | false ->
      incr_active server;
      let logev = Logev.enter_debug ~log ~__FILE__ ~__LINE__ "httpev.handle-client"
        ~attrs:["client.sockaddr", `String (Nix.show_addr sockaddr); "request.id", `Int req_id] in
      let client = { fd; req_id; sockaddr; time_conn=Time.get (); server; logev; req=Headers (Buffer.create 1024); } in
...
let setup_fd_lwt fd config answer =
  ...
  handle_lwt config fd begin fun (fd,sockaddr) ->
    ...
    let client =
      ...
      { fd = Lwt_unix.unix_file_descr fd; req_id; sockaddr; time_conn=Time.get (); logev; server; req=Headers (Buffer.create 0); }
    in
    if config.debug then log #info "accepted #%d %s" req_id (Nix.show_addr sockaddr);
    incr_active server;
    Hashtbl.replace server.clients req_id client;
    ...
    end [%lwt.finally decr_active server; Hashtbl.remove server.clients req_id; Logev.exit logev; BuffersCache.release buffer; Lwt.return_unit; ]
  end
